var Accounts = require("./../packages/web3-vap-accounts/src/index.js");
...
// tests from https://github.com/Gustav-Simonsson/go-vapory/blob/7cc6b801e0967e5ebfa26b9f670675acea6e3a20/accounts/testdata/v3_test_vector.json
var staticTests = [{
    "json": {
        "crypto" : {
            "cipher" : "aes-128-ctr",
...
describe("eth", function () {
    describe("accounts", function () {
        ...
        tests.forEach(function (test, i) {
            it("encrypt vap.account, and compare to vaporyjs-wallet", function() {
                ...
                assert.deepEqual(acc.encrypt(pw, {n: n,salt: salt, iv: iv, uuid: uuid}), vapWall.toV3(pw, {n: n, salt: salt, iv: iv, uuid: uuid}));
            });

            it("encrypt vap.account, and decrypt with vaporyjs-wallet", function() {
                var vapAccounts = new Accounts();
                ...
                // create vaporyjs-wallet account
                var vapWall = vaporyWallet.fromV3(encrypt, pw);
            ...
            it("encrypt vaporyjs-wallet, and decrypt with vap.account", function() {
                var vapAccounts = new Accounts();

                // create account
                var vapWall = vaporyWallet.generate();
                var encrypt = vapWall.toV3(pw, {n: n});

                // create vaporyjs-wallet account
                var acc = vapAccounts.decrypt(encrypt, pw);
            ...
            it("decrypt static signature using vaporyjs-wallet and vap.account and compare", function() {
                var vapAccounts = new Accounts();
                ...
                var acc = vapAccounts.decrypt(encrypt, pw);
                var vapWall = vaporyWallet.fromV3(encrypt, 'test');
        ...
        staticTests.forEach(function (test, i) {
            it("decrypt staticTests and compare to private key", function() {
                ...
                // create account
                var acc = vapAccounts.decrypt(test.json, test.password);
